...
DMRcaller <- function(m_ecotype, m_context){
	...
	filelist <- list.files(paste("output_dir/",newFolder,sep=""),
	...
	write.table(data.frame(meth_cpg_dmr_filtered@data.chr,
						...
						paste("input_dir/",newFolder,"/methylsig_DMRs_",newFolder,"_200bp.bed",sep=""), 
						row.names=F,col.names=F,sep="\t",quote=F,dec = ",")
}

mergeDmrs <- function(m_ecotype,m_context){
	dmrs_to_merge <- read.table(paste("input_dir//",m_ecotype,"_",m_context,"/DMRs_200bp.bed",sep="")
	...
	write.table(mergedDMRs,paste("input_dir/",m_ecotype,"_",m_context,
								"/mergedDMRs_",m_ecotype,"_",m_context,sep=""),
				row.names=F,col.names=F,sep="\t",quote=F)
}

peakRAM(DMRcaller("arabidopsis", "CpG"))
peakRAM(DMRcaller("arabidopsis", "CHG"))
peakRAM(DMRcaller("arabidopsis", "CHH"))


mergeDmrs("arabidopsis", "CpG")
mergeDmrs("arabidopsis", "CHG")
mergeDmrs("arabidopsis", "CHH")

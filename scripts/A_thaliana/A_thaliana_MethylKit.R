...
fitData <- function(dfMethData,m_filename,m_context,dirName){
	# a unique id, chromosome name and strand is missing
	...
	#setwd(paste(dirName,m_filename,"_",m_context,sep=""))
...
pdf(paste("output_dir/",newFolder,"PCA_",m_ecotype,"_",m_context, "comp12.pdf",sep=""))
	...
	pdf(paste("output_dir/",newFolder,"PCA_",m_ecotype,"_",m_context, "comp13.pdf",sep=""))
	pca_summary <- PCASamples(MethylRawList_united, obj.return = T, adj.lim=c(1.5,0.5),comp = c(1,3))
	dev.off()


	# Doing with dss package. Supposedly better.
	...
	write.table(sigDMR,file=paste("output_dir/",newFolder,"DMR_",m_context,"_methylkitDSS.txt",sep=""),
				row.names=F,col.names=T,sep="\t",quote=F)

	options("scipen"=50) # avoid scientific name convention

	# read out the original dmrs (not merged)
	write.table(sigDMR[1:3],file = paste("output_dir/",newFolder,m_ecotype, "_DMRs_", m_context,".bed",sep=""),
				row.names=F,col.names=F,sep="\t",quote=F)

	# read out the merged dmrs
	mergedDMRs <- mergedDMRs[-1,]
	mergedDMRs <- cbind("chr1",mergedDMRs)
	write.table(mergedDMRs,file=paste("output_dir/",newFolder,"methylkit_DMRs_merged_",m_ecotype,"_",m_context,".bed",sep=""),
				row.names=F,col.names=F,sep="\t",quote=F)
...
sim_chg_arabidopsis <- read.delim("input_dir//arabidopsis_CHG_SimulationData.txt",colClasses=c("integer",rep(c("integer","numeric"),6)))
sim_chh_arabidopsis <- read.delim("input_dir//arabidopsis_CHH_SimulationData.txt",colClasses=c("integer",rep(c("integer","numeric"),6)))
sim_cpg_arabidopsis <- read.delim("input_dir//arabidopsis_CpG_SimulationData.txt",colClasses=c("integer",rep(c("integer","numeric"),6)))


fitData(sim_chg_arabidopsis,"arabidopsis","CHG","/input_dir/")
fitData(sim_chh_arabidopsis,"arabidopsis","CHH","/input_dir/")
fitData(sim_cpg_arabidopsis,"arabidopsis","CpG","/input_dir/")


arabidopsis_chh_methdiffDSS = peakRAM(DMR_caller("arabidopsis", "CHG"))
arabidopsis_chg_methdiffDSS = peakRAM(DMR_caller("arabidopsis", "CHH"))
arabidopsis_cpg_methdiffDSS = peakRAM(DMR_caller("arabidopsis", "CpG"))
